...
Criacao e edicao DEVEM aceitar exclusivamente `{ "name": "..." }`. Campos adicionais, corpo ausente, UUID malformado ou nome invalido DEVEM retornar `400 Bad Request` sem persistir dados. Criacao DEVE retornar `201 Created` com `Location` e somente dados minimos; edicao DEVE retornar `200 OK`; desativacao efetiva DEVE retornar `204 No Content`.
