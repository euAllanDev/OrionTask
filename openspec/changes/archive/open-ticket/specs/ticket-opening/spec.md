# Ticket Opening Specification Delta
...
`POST /api/v1/organizations/{organizationId}/tickets` MUST exigir sessao server-side persistida e CSRF valido. `OWNER`, `ADMIN` e `TECHNICIAN` DEVEM poder abrir ticket. `creatorAccountId` DEVE ser obtido exclusivamente da sessao persistida; cliente NAO DEVE poder enviarlo ou substitui-lo.
